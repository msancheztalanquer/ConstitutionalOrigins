...
import delimited https://raw.githubusercontent.com/msancheztalanquer/ConstitutionalOrigins/main/labels.csv, varnames(1) clear
...
coefplot, keep(treated_year1 treated_year2 treated_year3 treated_year4 treated_year5 treated_year6 treated_year7 treated_year8 treated_year9 treated_year10) ///
 yline(0) vertical levels (99 95) ysc(r(-.1 .2)) ylabel(-.1(.05).2) xlabel(, angle(45) labsize(small)) ytitle("Liberal democracy index") xtitle("Years since enactment")
...
marginsplot, xlabel(, angle(45) labsize(small)) xtitle("Years before beginning of constitution-making process and after approval")
...
**Note: Data were collapsed to produce TABLE A5. Reload full data set before proceeding
import delimited https://raw.githubusercontent.com/msancheztalanquer/ConstitutionalOrigins/main/ConstitutionalOrigins_Data.csv, clear
do labels.do
label values decad* "decade"
label values yearin "yearinconstitutionmaking"
...
****TABLE A6. DiD Estimates of the Effect of Constitution-Making Modalities on Liberal Democracy, 1900 - 2015. Ordinal coding of plural approval and direct popular participation variables.
...
marginsplot, yscale(alt) xtitle("Plural competition") addplot(hist pluralcompetition, yaxis(2))
...
margins afterconstitution, at(pluralcompetition=(0(.1)1))
marginsplot, yscale(alt) xtitle("Plural competition") addplot(hist pluralcompetition if democraticconstitution == 1, yaxis(2))
...
treated_year10) yline(0) vertical levels (99 95) xlabel(, angle(45) labsize(small)) ysc(r(-.1 .2)) ylabel(-.1(.05).2) ///
ytitle("Liberal democracy index") xtitle("Years since enactment")
...
coefplot, keep(quinquennium*_pluralapproval) vertical level(95) yline(0) xline(1.5, lpattern(dash)) xlabel(, angle(45) labsize(small)) ytitle("Parameter estimate")
...
coefplot, keep(treated_placeboyear*) vertical level(95) yline(0) ytitle("Parameter estimate") xlabel(, angle(45) labsize(small))
